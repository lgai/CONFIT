...
# real data options
parser.add_argument("--traits", type=str, nargs='*', default=None) # file name of each trait, without filetype extension
...
parser.add_argument("--gwasFormat", type=str, default="pylmm") # default is pylmm (https://github.com/nickFurlotte/pylmm)
# can also use "UKBiobank_neale" (https://github.com/Nealelab/UK_Biobank_GWAS/blob/master/README.md#summary-stat-output), but snps must be sorted to all be in same order
...
parser.add_argument("--avg1active",type=int,default=0) # whether to average over configs w 1 active trait
...
if args.useSimulatedData == False:
    if args.Sigma_Z_file == None:
        msg = "Must specify Sigma_Z_file location if using real data with --Sigma_Z_file"
        raise argparse.ArgumentTypeError(msg)
    if args.traitPath == None:
        msg = "Must specify directory containing GWAS summary statistics with --traitPath"
        raise argparse.ArgumentTypeError(msg)
...
if args.useSimulatedData:
    ...
    gwasZscores, gwasPvals, Sigma_Z = readInGwasData(args.nSnp,
       args.traits, args.Sigma_Z_file, gwasFormat = args.gwasFormat,
       outdir=args.outdir, exptName=args.exptName) # also writes estimated Sigma_Z to file
    if args.verbose()
...
if args.initialPriorFile:
    ...
    weights_A = estimateWtsFromGwas(gwasPvals, configs, thresh = args.countPriorThresh, minPrior=1e-8, avg1active = args.avg1active)
